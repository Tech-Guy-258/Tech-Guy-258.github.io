          ...
          <div className="p-6 bg-white border-t border-gray-100 shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.05)] z-20">
             <div className="mb-4 relative">
                ...
                <div className="flex gap-2">
                   <div className="relative flex-1">
                      ...
                      />
                      {selectedCustomer && (
                         <button onClick={() => { setSelectedCustomer(null); setCustomerSearch(''); }} className="absolute right-2 top-1/2 -translate-y-1/2 text-purple-400 hover:text-purple-600"><X size={16} /></button>
                      )}
                   </div>
                   <button onClick={() => setShowNewCustomerModal(true)} className="bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl w-10 flex items-center justify-center transition-colors border border-gray-200"><Plus size={20} /></button>
                </div>
             </div>
             ...
             <div className="mb-4">
                ...
                <div className="grid grid-cols-4 gap-2">
                   <button onClick={() => setPaymentMethod('cash')} className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${paymentMethod === 'cash' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}><Wallet size={20} /><span className="text-[10px] font-bold mt-1">Num.</span></button>
                   <button onClick={() => setPaymentMethod('mpesa')} className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${paymentMethod === 'mpesa' ? 'bg-red-50 border-red-500 text-red-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}><Smartphone size={20} /><span className="text-[10px] font-bold mt-1">M-Pesa</span></button>
                   <button onClick={() => setPaymentMethod('emola')} className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${paymentMethod === 'emola' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}><Smartphone size={20} /><span className="text-[10px] font-bold mt-1">E-Mola</span></button>
                   <button onClick={() => setPaymentMethod('card')} className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${paymentMethod === 'card' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}><CreditCard size={20} /><span className="text-[10px] font-bold mt-1">POS</span></button>
                </div>
             </div>

            <div className="flex justify-between items-center mb-6">
              <span className="text-gray-500 font-medium">Total a Pagar</span>
              <span className="text-3xl font-bold text-gray-900">{symbol} {cartTotal.toFixed(2)}</span>
            </div>
            <button
              onClick={handleCheckout}
              disabled={cart.length === 0}
              className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg flex items-center justify-center"
            >
              <Receipt className="mr-2" size={20} /> Cobrar
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-[fadeIn_0.2s]">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-[scaleIn_0.2s_ease-out] relative">
            <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 p-8 text-center text-white relative">
              <div className="bg-white/20 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 backdrop-blur-sm shadow-inner"><CheckCircle size={32} /></div>
              ...
              <button onClick={() => setReceipt(null)} className="absolute top-4 right-4 text-white/60 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors"><X size={20} /></button>
            </div>
            
            <div className="p-6 relative">
              <div className="mt-2 text-center mb-4">
                  <span className="inline-block px-3 py-1 bg-gray-100 rounded-full text-xs font-bold text-gray-600 uppercase tracking-wide">{getMethodLabel(receipt.method)}</span>
                  {receipt.customer && <div className="mt-2 text-xs font-bold text-purple-600 flex items-center justify-center"><UserIcon size={12} className="mr-1" /> {receipt.customer.name}</div>}
              </div>
                ...
                {receipt.records.map((rec) => (
                  <div key={rec.id} className="flex justify-between items-center py-2 border-b border-dashed border-gray-100 last:border-0">
                    <div className="flex items-center gap-3">
                       <span className="bg-gray-100 text-gray-600 text-xs font-bold h-6 w-6 rounded flex items-center justify-center">{rec.quantity}</span>
                       <div>
                          <span className="text-sm text-gray-800 font-bold block leading-tight">{rec.itemName}</span>
                       </div>
                    </div>
                    <span className="text-sm text-gray-800 font-bold">{symbol} {(rec.totalRevenue * rate).toFixed(2)}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 space-y-2">
                 <div className="flex justify-between items-center">
                   ...
                   <span className="text-2xl font-extrabold text-emerald-600">{symbol} {receipt.total.toFixed(2)}</span>
                 </div>
                 <div className="flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest pt-2 border-t border-slate-200">
                    <span>Data e Hora</span>
                    <span>{receipt.timestamp}</span>
                 </div>
              </div>

              <button onClick={() => setReceipt(null)} className="mt-6 w-full bg-gray-900 text-white font-bold py-3.5 rounded-xl hover:bg-black transition-all shadow-lg flex items-center justify-center"><Receipt size={18} className="mr-2" /> Nova Venda</button>
            </div>
          </div>
        </div>
      )}
