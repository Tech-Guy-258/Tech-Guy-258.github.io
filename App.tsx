      ...
      <main className="flex-1 md:ml-72 w-full min-h-screen flex flex-col">
        ...
        <div className="flex-1">
          <Routes>
            ...
            <Route path="/" element={can('VIEW_REPORTS') ? <Dashboard items={activeBusiness.items} sales={activeBusiness.sales} logs={activeBusiness.auditLogs} expenses={activeBusiness.expenses} appointments={activeBusiness.appointments} customers={activeBusiness.customers} currency={currency} exchangeRates={rates} onCompleteAppointment={handleCompleteAppointment} onAddCustomer={handleAddCustomer} onPayExpense={handlePayExpense} onSaveAppointment={(appt) => updateActiveBusiness(b => ({...b, appointments: [...(b.appointments || []), appt]}))} onUpdateAppointmentStatus={(id, s) => updateActiveBusiness(b => ({...b, appointments: b.appointments?.map(a => a.id === id ? {...a, status: s} : a)}))} onSaveExpense={(e) => updateActiveBusiness(b => ({...b, expenses: [...(b.expenses || []).filter(ex => ex.id !== e.id), e]}))} activeBusinessName={activeBusiness.name} currentOperator={currentSession.operator.name} /> : <Navigate to="/profile" replace />} />
